...
import { Award, LogIn, Mail, KeyRound, Loader2 } from "lucide-react";
import {
  Card,
  CardContent,
  CardDescription,
...
export default function LoginPage() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    // Pode-se adicionar um router.push('/') aqui se o onAuthStateChanged não redirecionar automaticamente
    // ou se quisermos um redirecionamento imediato após o popup fechar.
    // Por ora, deixaremos o onAuthStateChanged gerenciar.
  };
  ...
  return (
    <Card className="w-full max-w-md shadow-xl">
      ...
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input type="password" placeholder="Sua senha" {...field} disabled={authLoading || formLoading} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <Button type="submit" className="w-full" disabled={authLoading || formLoading}>
              {formLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <LogIn className="mr-2 h-4 w-4" />}
              Entrar
            </Button>
          </form>
        </Form>
        ...
        <Button variant="outline" className="w-full mt-4" onClick={handleGoogleSignIn} disabled={authLoading || formLoading}>
